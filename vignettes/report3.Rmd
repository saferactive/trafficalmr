...
- Calculate the exposure in terms of distanced walked/cycled
...
We have adjusted the plots to show KSI rate of active travellers per 100,000 people (left map). The KSI rate varies significantly with 90% of LAs being between 8.2 and 24.1 per 100,000 people. Scotland has notably low KSI rates while, Cumbria, has a very high rate.
...
Based on these population-based measures, the City of London is an outlier due to it very low resident population, highlighting the importance of denominator selection in road safety research.
In the case of the City of London, the resident population of about 2,500 is not representative of the amount active travel. It is notable that Cumbria is a popular tourist destination and may have more active travel than is represented by its population.
...
If the 'proportion of KSI' represents a genuine difference in safety, then LAs with a high ratio (dark green in Figure \@ref(fig:slight-ksi)) are having proportionally more slight casualties than serious and fatal ones. This could be use to more segregation of active travellers from motor traffic, lower traffic speed or greater driver awareness of active travellers.
...
Looking at absolute number of KSI, the top 10 and bottom 10 are the same in 2010 and 2019 suggesting no overall change. The plot blow shows the Active Travel KSI rate per 100,000 workplace population from 2010 to 2019. The majority of LAs are shown with the thin grey lines, while the top 5 and bottom 5 LAs are highlighted in colour
...
It is clear from the results that outer London is a more dangerous place to cycle than inner London. However, the extent to which places with high crash rates per KM are an artefact of low cycling levels in that area or are real places of concern is not totally clear.
There are at least three ways of dealing with this issue, that places with low estimated levels of exposure will tend to have more variable crash rates with less certainty:

- Only focus on areas with a level of exposure (cycling km in this case) sufficiently high, with some cut-off (e.g. 1000 km per 500m grid cell per year minimum).
...
Mean change in cycle flows over the years 2010-2015 is shown for all DfT count points in London in Figure \@ref(fig:early-year). 
...
By assigning each grid point to a London Borough, we can then investigate changes in cycling uptake at the Borough level. These can be seen in Figure \@ref(fig:borough-preds). ***change this to use weighted values***
...
```{r crashes-speed, fig.cap="Plots of road crashes by road type – comparison of all versus active-only crashes. Top row: rural, bottom row: urban. Recorded crashes in GB 2017-2019.", out.width="45%", fig.show='hold'}
knitr::include_graphics(c(
...
- Visualise the results in an interactive web application: we have a scalable prototype of an interactive web application that is ready to add any of the outputs above. For instance, currently it shows a cycling level route network overlaid with points from STATS19 (the latter only London at the moment). We are looking at various visualization methods to present the outputs and allow users to use the tool to query the data. Figure 5.1 shows what the "test deployment" looks like right now.

```{r web-app, fig.cap="A screen shot of current test deployment of our interactive web application", out.width="95%", fig.show='hold'}
knitr::include_graphics(c(
