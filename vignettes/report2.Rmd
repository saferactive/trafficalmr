...
For count points with complete annual data (Figure \@ref(fig:road-cat)), the increase in cycling uptake during the years 2010-2014 closely corresponds with counts from minor roads ('B', 'C' and unclassified roads).
...
<add in another figure showing number of counts by road category>
...
knitr::include_graphics("https://raw.githubusercontent.com/saferactive/saferactive/master/figures/change-by-la.png")
```

The results presented in Figure \@ref(la-change) can also be represented as an animated map, as shown in Figure \@ref(fig:la-gif).
...
## General Additive Models of AADF 

General Additive Models were chosen for their flexibility to accommodate non-linear trends in spatial and temporal variables, and the ability to include random effects within the model.
...
The definition of a 'junction' depends on a number of factors, including the way that junctions are represented in the input data and, when aggregagating nearby junction entities e.g. on a roundabout, the threshold distance beyond which two junctions are treated separately.
...
These are at a conceptual but each could be implemented based on the data we have.
Scenarios could be implemented either as area-wide interventions or (more challenging) as specific intervention on specific roads (e.g. Chapletown Road in Leeds becomes one way for motor traffic freeing-up space for a 2 way cycleway and increased space for walking).
